...
def remove_nils_and_false_from_array(array)
	array.compact.delete_if {|e| e == false}
end
...
def every_possible_pairing_of_students(array)	15
...
def all_elements_except_first_3(array)
	array.reject {|e| array.index(e) < 3}
end
...
def get_first_half_of_string(string)
	limit = string.length.even? ? ((string.length / 2) - 1) : ((string.length - 1) / 2)
...
def number_of_elements_that_are_palindromes(array)
	array.keep_if {|e| e == e.reverse }.count
end
...
def get_all_letters_in_array_of_words(array)
	array.map {|word| word.scan(/\w/)}.flatten.sort
end
...
def titleize_a_string(string)
	array = string.scan(/\w+/).map do |word|
		exceptions = %w(and the of)
...
def is_a_2014_bank_holiday?(date)
	require 'nokogiri'
	require 'open-uri'
	page = Nokogiri::HTML(open('https://www.gov.uk/bank-holidays'))
	page.css('td.calendar_date')[0..4].each do |date_cell|
		return true if date_cell.content == date.strftime("%e %B")
	end
	false
end
